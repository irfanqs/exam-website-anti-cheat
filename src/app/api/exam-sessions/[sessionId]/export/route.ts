...
export async function GET(
  ...
  const rows: (string | number | null)[][] = [
    ["No", "Pertanyaan", "Tipe Soal", "Jawaban Saya", "Skor Diperoleh", "Total Poin"],
  ];

  examSession.exam.questions.forEach((q, i) => {
    ...
    rows.push([i + 1, q.text, TYPE_LABELS[q.type] ?? q.type, answerText, scoreLabel, q.points]);
  });

  return csvResponse(`jawaban-saya-${examSession.exam.code}.csv`, toCsv(rows));
}
